...
from core.preferences import extract_preference, format_preferences_for_prompt  # noqa: E402
...
def get_user_id() -> str:
    """
    Real user identification, replacing the Phase 3-7 hardcoded 'you'.
    Simple name-based login for now -- Phase 9+ can upgrade this to
    real session/device detection over xrdp if needed.
    """
    name = input("Who's this? (your name, or press Enter for 'you'): ").strip().lower()
    return name.replace(" ", "_") if name else "you"
...
def main():
    ...
    while True:
        ...
        # Quietly check for a stated preference on every message,
        # regardless of what the router classifies it as.
        pref = extract_preference(user_input, router_model=config["router_model"])
        if pref:
            category, value = pref
            save_preference(user_id, category, value)
            print(f"[Noted preference: {category} = {value}]")
